...
export const createTicket = (req, res) => {
  ...
  console.log(company_name, company_short_name);
  const { subject, ticket_body, client_id, priority_id, subdivision_id } =
    req.body;
  console.log(subject, ticket_body, client_id, priority_id, subdivision_id);
  const screenshot = req.file; // File uploaded via multer
  ...
  db.query(checkClientQuery, [client_id], (err, clientResults) => {
    ...
    if (clientResults.length === 0) {
      return res
        .status(400)
        .send("Invalid client_id. The client does not exist.");
    }
    ...
    db.query(checkPriorityQuery, [priority_id], (err, priorityResults) => {
      ...
      if (priorityResults.length === 0) {
        return res
          .status(400)
          .send("Invalid priority_id. The priority level does not exist.");
      }
      ...
      db.query(
        ...
        (err, subdivisionResults) => {
          ...
          if (subdivisionResults.length === 0) {
            return res
              .status(400)
              .send("Invalid subdivision_id. The subdivision does not exist.");
          }
                  ...
                  db.query(
                    ...
                    (err, emailResults) => {
                      ...
                      if (emailResults.length === 0) {
                        return res
                          .status(400)
                          .send("No email information found.");
                      }
